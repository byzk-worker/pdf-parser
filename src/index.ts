...
class Parser extends ReaderParserAbstract {
  ...
  public async signSealKeyword(sealId: string, pwd: string, keyword: string): Promise<void> {
    if (!pwd) {
      throw new Error("密码不能为空");
    }
    ...
    await sealInKeyword({ sealId, fileId: this._fileUploadInfo.id, pwd, keyword });
